...
\begin{longtable}{@{}ll@{}}
\caption{Notation used in the problem formulation.}\label{ch4:tab:notations}\\
\toprule
\endfirsthead
\toprule
\endhead
...
\begin{equation}
\label{ch4:eq:3}
p_{i,1} = p_{i}^{0} - \sum\limits_{k \in K}y_{i,1,k}^{\text{u}^{-}} + \sum\limits_{k \in K}y_{i,1,k}^{\text{u}^{+}} + \sum\limits_{m \in R}g_{i,1,m},\forall i \in S_{0},
\end{equation}

\begin{equation}
\label{ch4:eq:4}
p_{i,t} = p_{i,t - 1} - \sum\limits_{k \in K}y_{i,t,k}^{\text{u}^{-}} + \sum\limits_{k \in K}y_{i,t,k}^{\text{u}^{+}} + \sum\limits_{m \in R}g_{i,t,m},\forall i \in S_{0},t = 2,...,T',
\end{equation}

\begin{equation}
\label{ch4:eq:5}
b_{i,1} = b_{i}^{0} - \sum\limits_{k \in K}y_{i,1,k}^{\text{b}^{-}} + \sum\limits_{k \in K}y_{i,1,k}^{\text{b}^{+}} - \sum\limits_{m \in R}g_{i,1,m},\forall i \in S_{0},
\end{equation}

\begin{equation}
\label{ch4:eq:6}
b_{i,t} = b_{i,t - 1} - \sum\limits_{k \in K}y_{i,t,k}^{\text{b}^{-}} + \sum\limits_{k \in K}y_{i,t,k}^{\text{b}^{+}} - \sum\limits_{m \in R}g_{i,t,m},\forall i \in S_{0},t = 2,\ldots,T',
\end{equation}

\begin{equation}
\label{ch4:eq:7}
p_{i,t} + b_{i,t} \leq C_{i},\forall i \in S,t = 1,....T',
\end{equation}

\begin{equation}
\label{ch4:eq:8}
\sum\limits_{j \in S_{0}}\psi_{0,j,1,k}^{\text{u}} = y_{0,1,k}^{\text{u}^{-}},\forall k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:9}
\sum\limits_{j \in S_{0}}\psi_{i,j,t,k}^{\text{u}} = \sum\limits_{j \in S_{0},t - t_{j,i}' > 0}\psi_{j,i,t - t_{j,i}',k}^{\text{u}} - y_{i,t,k}^{\text{u}^{+}} + y_{i,t,k}^{\text{u}^{-}},\forall i \in S_{0},t = 2,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:10}
\psi_{i,j,T',k}^{\text{u}} = 0,\forall i \in S_{0},j \in S_{0},k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:11}
\psi_{i,j,t,k}^{\text{u}} \leq Q_{k}x_{i,j,t,k}^{\text{v}},\forall i \in S_{0},j \in S_{0},t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:12}
\psi_{i,j,1,k}^{\text{b}} = 0,\forall i \in S_{0},j \in S_{0},k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:13}
\sum\limits_{j \in S_{0}}\psi_{i,j,t,k}^{\text{b}} = \sum\limits_{j \in S_{0},t - t_{j,i}' > 0}\psi_{j,i,t - t_{j,i}',k}^{\text{b}} - y_{i,t,k}^{\text{b}^{+}} + y_{i,t,k}^{\text{b}^{-}},\forall i \in S_{0},t = 2,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:14}
\psi_{i,j,T',k}^{\text{b}} = 0,\forall i \in S_{0},j \in S_{0},k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:15}
\psi_{i,j,t,k}^{\text{b}} \leq Q_{k}x_{i,j,t,k}^{\text{v}},\forall i \in S_{0},j \in S_{0},t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:16}
\psi_{i,j,t,k}^{\text{u}} + \psi_{i,j,t,k}^{\text{b}} \leq Q_{k},\forall i \in S_{0},j \in S_{0},t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:17}
y_{i,t,k}^{\text{u}^{+}} \leq \min\left\{ {C_{i},Q_{k}} \right\} \cdot \sum\limits_{j \in S_{0}}x_{i,j,t,k}^{\text{v}},\forall i \in S_{0},t = 1,...,T',k \in K,
\end{equation}
...
\begin{equation}
\label{ch4:eq:19}
y_{0,t,k}^{\text{b}^{+}} \leq Q_{k} \cdot \sum\limits_{j \in S_{0}}x_{0,j,t,k}^{\text{v}},\forall t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:20}
y_{i,t,k}^{\text{b}^{+}} = 0,\forall i \in S,t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:21}
y_{0,t,k}^{\text{b}^{-}} = 0,\forall t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:22}
y_{i,t,k}^{\text{b}^{-}} \leq \min\left\{ {b_{i}^{0},Q_{k}} \right\} \cdot \sum\limits_{j \in S_{0}}x_{i,j,t,k}^{\text{v}},\forall i \in S,t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:23}
g_{0,t,m} = 0,\forall t = 1,...,T',m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:24}
g_{i,t,m} \leq b_{i}^{0} \cdot \sum\limits_{j \in S_{0}}x_{i,j,t,m}^{\text{r}},\forall i \in S,t = 1,...,T',m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:25}
\sum\limits_{t = 1}^{T'}{\sum\limits_{k \in K}y_{i,t,k}^{\text{b}^{-}}} + \sum\limits_{t = 1}^{T'}{\sum\limits_{m \in R}g_{i,t,m}} \leq b_{i}^{0},\forall i \in S,
\end{equation}

\begin{equation}
\label{ch4:eq:26}
\sum\limits_{j \in S_{0}}x_{0,j,1,k}^{\text{v}} = 1,\forall k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:27}
\sum\limits_{j \in S}x_{0,j,1,m}^{\text{r}} = 1,\forall m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:28}
\sum\limits_{i \in S_{0}}x_{i,0,T^{'} - t_{i,0}^{'},k}^{\text{v}} = 1,\forall k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:29}
\sum\limits_{i \in S_{0}}x_{i,0,T^{'} - t_{i,0}^{''},m}^{\text{r}} = 1,\forall m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:30}
\sum\limits_{j \in S_{0},t - t_{\mathit{ji}}' > 0}x_{j,i,t - t_{\mathit{ji}}',k}^{\text{v}} = \sum\limits_{u \in S_{0}}x_{i,u,t,k}^{\text{v}},\forall i \in S_{0},t = 2,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:31}
\sum\limits_{j \in S_{0},t - t_{\mathit{ji}}^{''} > 0}x_{j,i,t - t_{\mathit{ji}}^{''},m}^{\text{r}} = \sum\limits_{u \in S_{0}}x_{i,u,t,m}^{\text{r}},\forall i \in S_{0},t = 2,...,T',m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:32}
\left. \varsigma_{t,k}^{\text{v}} \geq \left( t - \partial_{k}^{\text{v}} + 1 \right)/T',\forall t = 1,\ldots,T',k \in K, \right.
\end{equation}

\begin{equation}
\label{ch4:eq:33}
\left. \varsigma_{t,k}^{\text{v}} \leq \left( t - \partial_{k}^{\text{v}} + 0.5 \right)/T' + 1,\forall t = 1,\ldots,T',k \in K, \right.
\end{equation}

\begin{equation}
\label{ch4:eq:34}
\left. \varsigma_{t,m}^{\text{r}} \geq \left( t - \partial_{m}^{\text{r}} + 1 \right)/T',\forall t = 1,\ldots,T',m \in R, \right.
\end{equation}

\begin{equation}
\label{ch4:eq:35}
\left. \varsigma_{t,m}^{\text{r}} \leq \left( t - \partial_{m}^{\text{r}} + 0.5 \right)/T' + 1,\forall t = 1,\ldots,T',m \in R, \right.
\end{equation}

\begin{equation}
\label{ch4:eq:36}
\sum\limits_{i,j \in S_{0}}\sum\limits_{t \leq w,t > t_{ij}^{'}}t_{i,j}^{\text{v}} \cdot x_{i,j,t - t_{ij}^{'},k}^{\text{v}} + \sum\limits_{i \in S_{0}}\sum\limits_{t \leq w}t^{\text{load}}\left( {y_{i,t,k}^{\text{u}^{+}} + y_{i,t,k}^{\text{u}^{-}} + y_{i,t,k}^{\text{b}^{+}} + y_{i,t,k}^{\text{b}^{-}}} \right) \leq w \cdot \tau,\forall w = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:37}
\sum\limits_{i,j \in S_{0}}\sum\limits_{t \leq w,t > 0}t_{i,j}^{\text{v}} \cdot x_{i,j,t,k}^{\text{v}} + \sum\limits_{i \in S_{0}}\sum\limits_{t \leq w}t^{\text{load}}\left( {y_{i,t,k}^{\text{u}^{+}} + y_{i,t,k}^{\text{u}^{-}} + y_{i,t,k}^{\text{b}^{+}} + y_{i,t,k}^{\text{b}^{-}}} \right) \geq \left( {w - 2} \right) \cdot \tau - \varsigma_{w,k}^{\text{v}}T,\forall w = 1,...,T^{'},k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:38}
\sum\limits_{i,j \in S_{0}}{\sum\limits_{t \leq w,t > t_{\mathit{ij}}^{''}}t_{i,j}^{\text{r}}} \cdot x_{i,j,t - t_{\mathit{ij}}^{''},m}^{\text{r}} + \sum\limits_{i \in S}{\sum\limits_{t \leq w}{t^{\text{repair}}g_{i,t,m}}} \leq w \cdot \tau,\forall w = 1,...,T',m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:39}
\sum\limits_{i,j \in S_{0}}{\sum\limits_{t \leq w,t > 0}t_{i,j}^{\text{r}}} \cdot x_{i,j,t,m}^{\text{r}} + \sum\limits_{i \in S}{\sum\limits_{t \leq w}{t^{\text{repair}}g_{i,t,m}}} \geq \left( {w - 2} \right) \cdot \tau - \varsigma_{w,m}^{\text{r}}T,\forall w = 1,...,T',m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:40}
x_{0,0,t,k}^{\text{v}} \leq 1 - \left( \varsigma_{t + 1,k}^{\text{v}} - \varsigma_{t,k}^{\text{v}} \right),\forall t = 1,\ldots,T' - 1,k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:41}
x_{0,0,t,m}^{\text{r}} \leq 1 - \left( \varsigma_{t + 1,m}^{\text{r}} - \varsigma_{t,m}^{\text{r}} \right),\forall t = 1,\ldots,T' - 1,m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:42}
x_{i,i,t,k}^{\text{v}} \leq \left( {y_{i,t,k}^{\text{u}^{+}} + y_{i,t,k}^{\text{u}^{-}} + y_{i,t,k}^{\text{b}^{+}} + y_{i,t,k}^{\text{b}^{-}}} \right) + \left\lfloor \frac{\tau}{t^{load}} \right\rfloor \varsigma_{t,k}^{\text{v}},\forall i \in S_{0},t = 1,\ldots,T^{'},k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:43}
x_{i,i,t,m}^{\text{r}} \leq g_{i,t,m} + \left\lfloor \frac{\tau}{t^{\text{repair}}} \right\rfloor \varsigma_{t,m}^{\text{r}},\forall i \in S_{0},t = 1,\ldots,T^{'},m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:44}
\sum\limits_{i \in S_{0}}{\sum\limits_{j \in S_{0}}x_{i,j,t,k}^{\text{v}}} \leq 1,\forall t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:45}
\sum\limits_{i \in S_{0}}{\sum\limits_{j \in S_{0}}x_{i,j,t,m}^{\text{r}}} \leq 1,\forall t = 1,...,T',m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:46}
\sum\limits_{j \in S_{0},j \neq i}{\sum\limits_{t = t_{j,i}^{''} + 1}^{T'}{\sum\limits_{m \in R}x_{j,i,t - t_{j,i}^{''},m}^{\text{r}}}} \leq 1,\forall i \in S_{0},
\end{equation}

\begin{equation}
\label{ch4:eq:47}
e_{i,j,t,k} \geq CER \cdot \left\lbrack {\rho_{0} \cdot x_{i,j,t,k}^{\text{v}} + \frac{\rho^{\ast} - \rho_{0}}{Q_{k}} \cdot \left( {\psi_{i,j,t,k}^{\text{u}} + \psi_{i,j,t,k}^{\text{b}}} \right)} \right\rbrack \cdot d_{\mathit{ij}},\mspace{100mu}\mspace{100mu}\forall i,j \in S_{0},t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:48}
x_{i,j,t,k}^{\text{v}} \in \left\{ 0,1 \right\},\forall i,j \in S_{0},t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:49}
x_{i,j,t,m}^{\text{r}} \in \left\{ 0,1 \right\},\forall i,j \in S_{0},t = 1,...,T',m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:50}
\varsigma_{t,k}^{\text{v}} \in \left\{ 0,1 \right\},\forall t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:51}
\varsigma_{t,m}^{\text{r}} \in \left\{ 0,1 \right\},\forall t = 1,...,T',m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:52}
p_{i,t} \geq 0,\forall i \in S_{0},t = 1,...,T',
\end{equation}

\begin{equation}
\label{ch4:eq:53}
b_{i,t} \geq 0,\forall i \in S_{0},t = 1,...,T',
\end{equation}

\begin{equation}
\label{ch4:eq:54}
y_{i,t,k}^{\text{u}^{+}} \geq 0,\text{integer},\forall i \in S_{0},t = 1,...,T',k \in K,
\end{equation}
...
\begin{equation}
\label{ch4:eq:56}
y_{i,t,k}^{\text{b}^{+}} \geq 0,\text{integer},\forall i \in S_{0},t = 1,...,T',k \in K,
\end{equation}
...
\begin{equation}
\label{ch4:eq:58}
g_{i,t,m} \geq 0,\text{integer},\forall i \in S_{0},t = 1,...,T',m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:59}
\partial_{k}^{\text{v}} \geq 0,\text{integer},\forall k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:60}
\partial_{m}^{\text{r}} \geq 0,\text{integer},\forall m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:61}
\partial_{k}^{\text{v}} \leq T',\text{integer},\forall k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:62}
\partial_{m}^{\text{r}} \leq T',\text{integer},\forall m \in R,
\end{equation}

\begin{equation}
\label{ch4:eq:63}
\psi_{i,j,t,k}^{\text{u}} \geq 0,\forall i,\mspace{100mu} j \in S_{0},t = 1,...,T',k \in K,
\end{equation}

\begin{equation}
\label{ch4:eq:64}
\psi_{i,j,t,k}^{\text{b}} \geq 0,\forall i,\mspace{100mu} j \in S_{0},t = 1,...,T',k \in K,\quad\mspace{600mu}\text{and}
\end{equation}

\begin{equation}
\label{ch4:eq:65}
e_{i,j,t,k} \geq 0,\forall i,\mspace{100mu} j \in S_{0},t = 1,...,T',k \in K.
\end{equation}
...
Constraints~\eqref{ch4:eq:3},~\eqref{ch4:eq:4},~\eqref{ch4:eq:5},~\eqref{ch4:eq:6} keep track of the numbers of usable and broken bikes at each node at
the end of each period. Constraints~\eqref{ch4:eq:7} stipulate that the total number of usable and broken bikes
at each station is no more than its capacity. Constraints~\eqref{ch4:eq:8} specify the initial load of usable
bikes on each truck during the first period. Constraints~\eqref{ch4:eq:9} update the load of usable bikes on each
truck at each node during each period. Constraints~\eqref{ch4:eq:10} require that each truck holds no usable
bikes at the end of the last period. Constraints~\eqref{ch4:eq:11} ensure that the quantity of usable bikes
carried by each truck in any period does not exceed its capacity. Similar constraints for broken
bikes are provided in constraints~\eqref{ch4:eq:12},~\eqref{ch4:eq:13},~\eqref{ch4:eq:14},~\eqref{ch4:eq:15}. Constraints~\eqref{ch4:eq:16} ensure that the total
...
Constraints~\eqref{ch4:eq:26},~\eqref{ch4:eq:27},~\eqref{ch4:eq:28},~\eqref{ch4:eq:29} require that both trucks and repairers depart from the depot
initially and return to the depot eventually. Constraints~\eqref{ch4:eq:30},~\eqref{ch4:eq:31} are the
flow~conservation~conditions for trucks and repairers, respectively. Constraints~\eqref{ch4:eq:32},~\eqref{ch4:eq:33},~\eqref{ch4:eq:34},
\eqref{ch4:eq:35} relate \(\varsigma_{t,k}^{\text{v}}\)(the variable used to define whether a truck or a repairer stays
...
Constraints~\eqref{ch4:eq:36},~\eqref{ch4:eq:37},~\eqref{ch4:eq:38},~\eqref{ch4:eq:39} restrict the upper and lower bounds of the operational time, and
are modified from the study of Liu et al. (2018). Specifically, constraints~\eqref{ch4:eq:36} imply that up to
...
constraints are also imposed on the repairers in constraints~\eqref{ch4:eq:38},~\eqref{ch4:eq:39}. Note that constraints~\eqref{ch4:eq:37},
\eqref{ch4:eq:39} are only activated before the period that the truck and the repairer return to the depot for
...
each period when traveling between any two nodes. Constraints~\eqref{ch4:eq:48},~\eqref{ch4:eq:49},~\eqref{ch4:eq:50},~\eqref{ch4:eq:51},~\eqref{ch4:eq:52},~\eqref{ch4:eq:53},
\eqref{ch4:eq:54},~\eqref{ch4:eq:55},~\eqref{ch4:eq:56},~\eqref{ch4:eq:57},~\eqref{ch4:eq:58},~\eqref{ch4:eq:59},~\eqref{ch4:eq:60},~\eqref{ch4:eq:61},~\eqref{ch4:eq:62},~\eqref{ch4:eq:63},~\eqref{ch4:eq:64},~\eqref{ch4:eq:65} are the domain constraints.
...
\label{ch4:eq:66}
{\hat{F}}_{i} \geq \alpha_{p_{i},b_{i}}^{i} \cdot p_{i,T^{'}} + \beta_{p_{i},b_{i}}^{i} \cdot b_{i,T^{'}} + \gamma_{p_{i},b_{i}}^{i},\forall\left( {p_{i},b_{i}} \right) \in \left\{ {\left( {p_{i},b_{i}} \right) \in {\mathbb{Z}}^{2} \mid p_{i} \geq 0,b_{i} \geq 0,p_{i} + b_{i} \leq C_{i}} \right\},\forall i \in S.
\end{equation}
...
The algorithmic procedure for HGSADC-SBC is given in Table~\ref{ch4:tab:hgsadc-sbc-procedure}. The algorithm starts with
...
of this distribution are depicted in Figure~\ref{ch4:fig:bcrf-time-allocation}, which are calculated as follows:\[\begin{array}{l}
...
As in the study of Li et al. (2016), the penalty coefficient \(\eta\) in equation~\eqref{ch4:eq:70} is modified every
...
2023). The hybrid algorithm HGSADC-SBC was implemented with C++. All numerical experiments were
...
\emph{\(\rho\)*} & The full-load fuel consumption rate of each truck (liters/km) & 0.258 & Franceschetti et
...
from the 518-station dataset. Specifically, in parameter tuning in Section~\ref{ch4:subsec:parameter-tuning-setting}, a 60-station network
was generated. In the experiment in Section~\ref{ch4:subsec:small-network-comparison}, five different instances were generated for each
network size (6, 10, and 15 stations). In the experiment in Section~\ref{ch4:subsec:large-network-comparison}, instances with network
sizes ranging from 60 to 500 stations were created, with one instance generated for each size. In
the experiment in Section~\ref{ch4:subsec:repairer-cost-effectiveness}, one of the 15-station networks generated in Section~\ref{ch4:subsec:small-network-comparison} was used. As
...
that repair times do not exceed the period as defined in the parameter table in Table~\ref{ch4:tab:notations}. The codes
...
HGSADC-SBC} & \multirow{2}{*}{Empty Cell} & \multirow{2}{*}{\textsuperscript{5}Gap (\%)} \\
...
Numerical experiments on small networks of up to 15 stations demonstrated that HGSADC-SBC
...
HGSADC-SBC outperformed Gurobi in finding a feasible solution obtained under a two-hour time limit,
